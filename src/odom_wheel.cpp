...
public:
          ...
          left_wheel_vel_(0.0), right_wheel_vel_(0.0)
    {
        ...
        // Publisher with explicit QoS settings to prevent message loss
        // Use Reliable QoS with larger queue depth for compatibility with EKF and debugging tools
        // This ensures messages aren't dropped when subscribers are slow to process
        rclcpp::QoS odom_qos(50);  // Increased queue depth to handle bursts
        odom_qos.reliability(rclcpp::ReliabilityPolicy::Reliable);
        odom_qos.durability(rclcpp::DurabilityPolicy::Volatile);
        odom_qos.keep_last(50);  // Keep last 50 messages
        odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("/odom_wheel", odom_qos);
        
        RCLCPP_INFO(this->get_logger(), "Publisher created for /odom_wheel (actual topic: %s)", 
                   odom_pub_->get_topic_name());
        RCLCPP_INFO(this->get_logger(), "Publisher subscription count: %zu", odom_pub_->get_subscription_count());
        ...
        RCLCPP_INFO(this->get_logger(), "TF publication disabled - EKF publishes odom -> base_link transform");
    }
    ...
    void publishOdometry()
    {
        ...
        // TF publication disabled - EKF publishes odom -> base_link transform
        // Publish TF: odom_wheel -> base_link (to avoid conflicts with EKF's odom -> base_link)
        // geometry_msgs::msg::TransformStamped odom_tf;
        // odom_tf.header.stamp = current_time;
        // odom_tf.header.frame_id = "odom_wheel";
        ...
        // Publish odometry (using odom_wheel frame to match TF)
        nav_msgs::msg::Odometry odom_msg;
        odom_msg.header.stamp = current_time;
        odom_msg.header.frame_id = "odom_wheel";
        ...
        try {
            // Log subscription count periodically to diagnose DDS discovery
            static int publish_count = 0;
            publish_count++;
            
            if (publish_count <= 5 || publish_count % 50 == 0) {
                size_t sub_count = odom_pub_->get_subscription_count();
                RCLCPP_INFO(this->get_logger(), 
                    "Publishing odometry #%d: x=%.3f, y=%.3f, yaw=%.3f, v=%.3f, omega=%.3f, subscribers=%zu", 
                    publish_count, x_, y_, yaw_, v, omega, sub_count);
            }
